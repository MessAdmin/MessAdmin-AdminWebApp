...
function checkUncheckAllCB(theElementCB, name) {
	...
	for(i = 0; i < elements.length; ++i) {
		theField = elements[i];
		if(theField.type == 'checkbox' && theField.name == name && !theField.disabled) {
...
function hasCheckedCB(theElementCB, name) {
	...
	for(i = 0; i < elements.length; ++i) {
		theField = elements[i];
		if((theField.type == 'checkbox' || theField.type == 'radio') && theField.name == name && theField.checked && !theField.disabled) {
			return true;
		}
	}
	return false;
...
addWindowOnLoadHandler(function () {
	try {
		var checkUpdateDone = RAM("checkUpdateFlag");
		if (! checkUpdateDone) {
			jah(checkUpdateURL, null, "HEAD", null, function() {
				RAM("checkUpdateValue", 1);
				showUpdateNotification();
			}, function() {
//				RAM("checkUpdateValue", 0);
//				showUpdateNotification();
			});
			RAM("checkUpdateFlag", 1);
		} else {
			showUpdateNotification();
		}
	} catch (e) {/*alert(e)*/}; // NoScript causes RAM("id") to fail
...
function showUpdateNotification() {
	try {
		var checkUpdateValue = RAM("checkUpdateValue");
		if (checkUpdateValue) {
			showElementWithId('updateNotification');
//		} else {
//			hideElementWithId('updateNotification');
		}
	} catch (e) {/*alert(e)*/}; // NoScript causes RAM("id") to fail
}
...
// Build a query string (without leading '?')
function buildFormQueryString(formName) {
...
function jah(url, target, method, params, callback200, callbackError) {
	...
	function prepareXMLHttpRequest() {
		...
		jah_req.setRequestHeader("X-Requested-With", "XMLHttpRequest");
